...
import { Plus, Snowflake, Refrigerator, Thermometer, Boxes, Settings, AlertCircle } from 'lucide-react'

const TEMP_CATEGORIES = [
  { value: 'Refrigerated', label: 'Refrigerated (1-6°C)', icon: Refrigerator, color: 'text-sky-600 bg-sky-50' },
  { value: 'Frozen', label: 'Frozen (-18°C and below)', icon: Snowflake, color: 'text-violet-600 bg-violet-50' },
  { value: 'Room Temperature', label: 'Room Temperature (20-24°C)', icon: Thermometer, color: 'text-amber-600 bg-amber-50' },
]
                      ...
                      <TableRow key={s.id}>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <div className={`w-8 h-8 rounded-md ${TEMP_CATEGORIES.find(t => t.value === s.tempCategory)?.color ?? 'bg-slate-100'} flex items-center justify-center`}>
                              <Icon className="w-4 h-4" />
                            </div>
                            <span className="text-sm font-medium">{s.name}</span>
                          </div>
                        </TableCell>
                        <TableCell><Badge variant="outline" className="border-slate-200">{s.tempCategory}</Badge></TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-600">{used}/{s.maxCapacity}</span>
                          </div>
                        </TableCell>
                      </TableRow>
                    )
...
function AddStorageDialog({ open, onOpenChange, onSuccess }: { open: boolean; onOpenChange: (v: boolean) => void; onSuccess: () => void }) {
  ...
  // Auto-set temperature based on category
