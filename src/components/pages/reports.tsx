...
export function ReportsPage() {
  ...
  async function printReport() {
    // Generate a professional PDF and open it in a new tab for printing
    // This avoids the browser's native print which adds URLs and has overlapping issues
    setExporting('pdf')
    try {
      const params = new URLSearchParams()
      if (from) params.set('from', from)
      ...
      if (!res.ok) {
        const data = await res.json()
        toast.error(data.error ?? 'Failed to generate printable report')
        return
      }
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      // Open in new tab - the browser's built-in PDF viewer has a proper print button
      // with no URL footer and no overlapping issues
      window.open(url, '_blank')
      // Revoke after a delay to allow the PDF to load
      setTimeout(() => URL.revokeObjectURL(url), 60000)
      toast.success('Report opened in new tab — use the PDF viewer\'s print button')
    } catch (e) {
